...
vcf_1 <- read.table("vcf_C_pyg_22_forward_indelRm_intersect_annotated_SNP_is_alt.bed")[,c(1,3,6,20)]
...
ggplot(vcf_1, aes(pos, chCADD)) +
  geom_point() +
  ggtitle("chCADD, scaffold 2, individual C_pyg_22", 
...
df = data.frame(wartość=vcf_1$chCADD, grupa=vcf_1$genotype)
...
```{r}
obs_diff <- mean(df$wartość[df$grupa == "HOM_ALT"]) - mean(df$wartość[df$grupa == "HET"])
...
for(i in 1:1000) {
  ...
  perm_diffs[i] <- mean(df$wartość[perm_groups == "HOM_ALT"]) - mean(df$wartość[perm_groups == "HET"])
}
